...
class QAChecker:
    ...
    def check_unit(self, source_text: str, target_text: str, unit_state: str) -> QAResult:
        ...
        source_tokens = set(self.valid_token_pattern.findall(source_text))
        target_tokens = set(self.valid_token_pattern.findall(target_text))
        ...
        if missing_tokens:
            sorted_missing = sorted(list(missing_tokens))
            qa_details["missing"] = sorted_missing
            issues.append(QAIssue(
                type="missing", 
                severity="error", 
                message=f"Missing tokens: {', '.join(sorted_missing)}",
                details=sorted_missing
            ))
            has_errors = True
            
        if extra_tokens:
            sorted_extra = sorted(list(extra_tokens))
